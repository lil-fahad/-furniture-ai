"""
Build furniture training catalog.

Combines the full IKEA and Alibaba curated catalogs into:
  datasets/furniture.csv     – flat CSV for the recommender trainer
  datasets/furniture.json    – full JSON with all fields

Also runs validate_alibaba() to verify catalog integrity.
"""
import csv
import json
import sys
from pathlib import Path
from typing import List

# ---------------------------------------------------------------------------
# Full IKEA catalog (mirrors backend/services/ikea_service.py)
# ---------------------------------------------------------------------------
IKEA_ITEMS: List[dict] = [
    {
        "item_id": "ikea-kivik-sofa", "source": "ikea",
        "name": "KIVIK Sofa", "category": "sofa", "price": 799.0, "currency": "USD",
        "styles": ["modern", "scandinavian", "minimalist"],
        "colors": ["beige", "grey", "dark blue", "white"],
        "dimensions": {"w": 228, "d": 95, "h": 83, "unit": "cm"},
        "rating": 4.6, "reviews": 1842,
        "image_url": "https://www.ikea.com/us/en/images/products/kivik-sofa-tibbleby-beige-grey__0727324_pe735595_s5.jpg",
        "buy_url": "https://www.ikea.com/us/en/p/kivik-sofa-tibbleby-beige-grey-s09408182/",
        "description": "A generous seating series with a soft, deep seat and comfortable support.",
    },
    {
        "item_id": "ikea-ektorp-sofa", "source": "ikea",
        "name": "EKTORP Sofa", "category": "sofa", "price": 649.0, "currency": "USD",
        "styles": ["classic", "traditional", "cozy"],
        "colors": ["white", "beige", "light grey"],
        "dimensions": {"w": 211, "d": 88, "h": 88, "unit": "cm"},
        "rating": 4.5, "reviews": 2310,
        "image_url": "https://www.ikea.com/us/en/images/products/ektorp-sofa-vittaryd-white__0182432_pe336842_s5.jpg",
        "buy_url": "https://www.ikea.com/us/en/p/ektorp-sofa-vittaryd-white-s29301175/",
        "description": "Traditional sofa with a timeless design and washable covers.",
    },
    {
        "item_id": "ikea-poang-chair", "source": "ikea",
        "name": "POÄNG Armchair", "category": "chair", "price": 129.0, "currency": "USD",
        "styles": ["scandinavian", "modern", "minimalist"],
        "colors": ["birch/beige", "birch/black", "brown/orange"],
        "dimensions": {"w": 68, "d": 82, "h": 100, "unit": "cm"},
        "rating": 4.7, "reviews": 3201,
        "image_url": "https://www.ikea.com/us/en/images/products/poaeng-armchair-birch-veneer-knisa-light-beige__0637597_pe698420_s5.jpg",
        "buy_url": "https://www.ikea.com/us/en/p/poaeng-armchair-birch-veneer-knisa-light-beige-s09202973/",
        "description": "Iconic layered-bent birch frame armchair with cushioned seat.",
    },
    {
        "item_id": "ikea-markus-chair", "source": "ikea",
        "name": "MARKUS Office Chair", "category": "chair", "price": 229.0, "currency": "USD",
        "styles": ["modern", "office", "minimalist"],
        "colors": ["dark grey", "black"],
        "dimensions": {"w": 62, "d": 60, "h": 128, "unit": "cm"},
        "rating": 4.8, "reviews": 5420,
        "image_url": "https://www.ikea.com/us/en/images/products/markus-office-chair-vissle-dark-grey__0603578_pe681588_s5.jpg",
        "buy_url": "https://www.ikea.com/us/en/p/markus-office-chair-vissle-dark-grey-90289172/",
        "description": "Ergonomic office chair with lumbar support and adjustable height.",
    },
    {
        "item_id": "ikea-lack-table", "source": "ikea",
        "name": "LACK Side Table", "category": "table", "price": 19.99, "currency": "USD",
        "styles": ["minimalist", "modern", "scandinavian"],
        "colors": ["black-brown", "white", "birch"],
        "dimensions": {"w": 55, "d": 55, "h": 45, "unit": "cm"},
        "rating": 4.3, "reviews": 4100,
        "image_url": "https://www.ikea.com/us/en/images/products/lack-side-table-black-brown__0178601_pe331783_s5.jpg",
        "buy_url": "https://www.ikea.com/us/en/p/lack-side-table-black-brown-80104268/",
        "description": "Simple and functional side table, perfect for small spaces.",
    },
    {
        "item_id": "ikea-lisabo-table", "source": "ikea",
        "name": "LISABO Dining Table", "category": "table", "price": 299.0, "currency": "USD",
        "styles": ["scandinavian", "natural", "modern"],
        "colors": ["ash veneer", "black"],
        "dimensions": {"w": 140, "d": 78, "h": 74, "unit": "cm"},
        "rating": 4.5, "reviews": 890,
        "image_url": "https://www.ikea.com/us/en/images/products/lisabo-table-ash-veneer__0728495_pe736408_s5.jpg",
        "buy_url": "https://www.ikea.com/us/en/p/lisabo-table-ash-veneer-s09281170/",
        "description": "Ash veneer dining table with a clean, natural look.",
    },
    {
        "item_id": "ikea-kallax-shelf", "source": "ikea",
        "name": "KALLAX Shelf Unit", "category": "storage", "price": 109.0, "currency": "USD",
        "styles": ["modern", "minimalist", "scandinavian"],
        "colors": ["white", "black-brown", "birch", "oak"],
        "dimensions": {"w": 77, "d": 39, "h": 147, "unit": "cm"},
        "rating": 4.7, "reviews": 6780,
        "image_url": "https://www.ikea.com/us/en/images/products/kallax-shelf-unit-white__0644757_pe702938_s5.jpg",
        "buy_url": "https://www.ikea.com/us/en/p/kallax-shelf-unit-white-s80275887/",
        "description": "Versatile shelf unit that can be used as room divider or TV bench.",
    },
    {
        "item_id": "ikea-pax-wardrobe", "source": "ikea",
        "name": "PAX Wardrobe", "category": "storage", "price": 249.0, "currency": "USD",
        "styles": ["modern", "minimalist", "classic"],
        "colors": ["white", "black-brown", "oak"],
        "dimensions": {"w": 100, "d": 60, "h": 201, "unit": "cm"},
        "rating": 4.6, "reviews": 3450,
        "image_url": "https://www.ikea.com/us/en/images/products/pax-wardrobe-white-bergsbo-white__0901218_pe658887_s5.jpg",
        "buy_url": "https://www.ikea.com/us/en/p/pax-wardrobe-white-bergsbo-white-s09156845/",
        "description": "Customizable wardrobe system with sliding or hinged doors.",
    },
    {
        "item_id": "ikea-malm-bed", "source": "ikea",
        "name": "MALM Bed Frame", "category": "bed", "price": 349.0, "currency": "USD",
        "styles": ["modern", "minimalist", "scandinavian"],
        "colors": ["white", "black-brown", "oak veneer"],
        "dimensions": {"w": 160, "d": 200, "h": 100, "unit": "cm"},
        "rating": 4.5, "reviews": 4200,
        "image_url": "https://www.ikea.com/us/en/images/products/malm-bed-frame-high-white__0637598_pe698421_s5.jpg",
        "buy_url": "https://www.ikea.com/us/en/p/malm-bed-frame-high-white-s19175478/",
        "description": "Clean-lined bed frame with high headboard and underbed storage.",
    },
    {
        "item_id": "ikea-hektar-lamp", "source": "ikea",
        "name": "HEKTAR Floor Lamp", "category": "lighting", "price": 79.99, "currency": "USD",
        "styles": ["industrial", "modern", "loft"],
        "colors": ["dark grey", "white"],
        "dimensions": {"w": 35, "d": 35, "h": 179, "unit": "cm"},
        "rating": 4.4, "reviews": 1230,
        "image_url": "https://www.ikea.com/us/en/images/products/hektar-floor-lamp-with-led-bulb-dark-grey__0728496_pe736409_s5.jpg",
        "buy_url": "https://www.ikea.com/us/en/p/hektar-floor-lamp-with-led-bulb-dark-grey-s29278584/",
        "description": "Industrial-style floor lamp with adjustable head.",
    },
    {
        "item_id": "ikea-adum-rug", "source": "ikea",
        "name": "ÅDUM Rug", "category": "rug", "price": 149.0, "currency": "USD",
        "styles": ["cozy", "modern", "scandinavian"],
        "colors": ["off-white", "grey", "beige"],
        "dimensions": {"w": 200, "d": 300, "h": 0, "unit": "cm"},
        "rating": 4.6, "reviews": 2100,
        "image_url": "https://www.ikea.com/us/en/images/products/adum-rug-high-pile-off-white__0728497_pe736410_s5.jpg",
        "buy_url": "https://www.ikea.com/us/en/p/adum-rug-high-pile-off-white-s00278578/",
        "description": "High-pile rug with a soft, luxurious feel underfoot.",
    },
    {
        "item_id": "ikea-micke-desk", "source": "ikea",
        "name": "MICKE Desk", "category": "desk", "price": 99.99, "currency": "USD",
        "styles": ["modern", "minimalist", "office"],
        "colors": ["white", "black-brown", "oak"],
        "dimensions": {"w": 105, "d": 50, "h": 75, "unit": "cm"},
        "rating": 4.4, "reviews": 3100,
        "image_url": "https://www.ikea.com/us/en/images/products/micke-desk-white__0736678_pe740627_s5.jpg",
        "buy_url": "https://www.ikea.com/us/en/p/micke-desk-white-s80213074/",
        "description": "Compact desk with integrated cable management and drawer.",
    },
]

# ---------------------------------------------------------------------------
# Full Alibaba catalog (mirrors backend/services/alibaba_service.py)
# ---------------------------------------------------------------------------
ALIBABA_ITEMS: List[dict] = [
    {
        "item_id": "ali-nordic-sofa-3s", "source": "alibaba",
        "name": "Nordic Fabric Sofa 3-Seater", "category": "sofa", "price": 320.0, "currency": "USD",
        "styles": ["nordic", "modern", "minimalist"],
        "colors": ["light grey", "dark grey", "beige", "blue"],
        "dimensions": {"w": 210, "d": 85, "h": 80, "unit": "cm"},
        "rating": 4.3, "reviews": 456,
        "image_url": "https://s.alicdn.com/@sc04/kf/H8b3e2e2e2e2e4e2e8e2e2e2e2e2e2e2eX.jpg_300x300.jpg",
        "buy_url": "https://www.alibaba.com/product-detail/Nordic-Fabric-Sofa_62534567890.html",
        "description": "Wholesale Nordic-style 3-seater fabric sofa, MOQ 1 unit.",
    },
    {
        "item_id": "ali-velvet-sofa", "source": "alibaba",
        "name": "Luxury Velvet Chesterfield Sofa", "category": "sofa", "price": 480.0, "currency": "USD",
        "styles": ["luxury", "classic", "vintage"],
        "colors": ["emerald green", "navy blue", "burgundy", "gold"],
        "dimensions": {"w": 195, "d": 90, "h": 85, "unit": "cm"},
        "rating": 4.5, "reviews": 312,
        "image_url": "https://s.alicdn.com/@sc04/kf/Hvelvet_sofa_chesterfield.jpg_300x300.jpg",
        "buy_url": "https://www.alibaba.com/product-detail/Velvet-Chesterfield-Sofa_62534567891.html",
        "description": "Premium velvet Chesterfield sofa with button tufting.",
    },
    {
        "item_id": "ali-eames-replica-chair", "source": "alibaba",
        "name": "Eames Style Lounge Chair & Ottoman", "category": "chair", "price": 185.0, "currency": "USD",
        "styles": ["mid-century", "modern", "luxury"],
        "colors": ["black/walnut", "brown/walnut", "white/walnut"],
        "dimensions": {"w": 83, "d": 84, "h": 93, "unit": "cm"},
        "rating": 4.4, "reviews": 789,
        "image_url": "https://s.alicdn.com/@sc04/kf/Heames_lounge_chair.jpg_300x300.jpg",
        "buy_url": "https://www.alibaba.com/product-detail/Eames-Lounge-Chair_62534567892.html",
        "description": "Mid-century modern lounge chair with genuine leather and walnut shell.",
    },
    {
        "item_id": "ali-mesh-office-chair", "source": "alibaba",
        "name": "Ergonomic Mesh Office Chair", "category": "chair", "price": 95.0, "currency": "USD",
        "styles": ["office", "modern", "ergonomic"],
        "colors": ["black", "grey", "blue"],
        "dimensions": {"w": 65, "d": 65, "h": 120, "unit": "cm"},
        "rating": 4.2, "reviews": 1230,
        "image_url": "https://s.alicdn.com/@sc04/kf/Hmesh_office_chair.jpg_300x300.jpg",
        "buy_url": "https://www.alibaba.com/product-detail/Ergonomic-Mesh-Chair_62534567893.html",
        "description": "Full mesh ergonomic chair with lumbar support and adjustable armrests.",
    },
    {
        "item_id": "ali-marble-dining-table", "source": "alibaba",
        "name": "Marble Top Dining Table", "category": "table", "price": 420.0, "currency": "USD",
        "styles": ["luxury", "modern", "italian"],
        "colors": ["white marble/gold", "black marble/silver"],
        "dimensions": {"w": 160, "d": 90, "h": 76, "unit": "cm"},
        "rating": 4.6, "reviews": 234,
        "image_url": "https://s.alicdn.com/@sc04/kf/Hmarble_dining_table.jpg_300x300.jpg",
        "buy_url": "https://www.alibaba.com/product-detail/Marble-Dining-Table_62534567894.html",
        "description": "Italian marble top dining table with stainless steel base.",
    },
    {
        "item_id": "ali-solid-wood-coffee-table", "source": "alibaba",
        "name": "Solid Wood Coffee Table", "category": "table", "price": 145.0, "currency": "USD",
        "styles": ["rustic", "natural", "scandinavian"],
        "colors": ["natural oak", "walnut", "ebony"],
        "dimensions": {"w": 120, "d": 60, "h": 45, "unit": "cm"},
        "rating": 4.5, "reviews": 567,
        "image_url": "https://s.alicdn.com/@sc04/kf/Hsolid_wood_coffee_table.jpg_300x300.jpg",
        "buy_url": "https://www.alibaba.com/product-detail/Solid-Wood-Coffee-Table_62534567895.html",
        "description": "Handcrafted solid oak coffee table with natural finish.",
    },
    {
        "item_id": "ali-tv-cabinet", "source": "alibaba",
        "name": "Modern TV Cabinet with LED", "category": "storage", "price": 210.0, "currency": "USD",
        "styles": ["modern", "minimalist", "contemporary"],
        "colors": ["white", "walnut", "grey"],
        "dimensions": {"w": 180, "d": 40, "h": 50, "unit": "cm"},
        "rating": 4.4, "reviews": 890,
        "image_url": "https://s.alicdn.com/@sc04/kf/Htv_cabinet_led.jpg_300x300.jpg",
        "buy_url": "https://www.alibaba.com/product-detail/TV-Cabinet-LED_62534567896.html",
        "description": "Floating TV cabinet with LED strip lighting and push-open doors.",
    },
    {
        "item_id": "ali-bookshelf-industrial", "source": "alibaba",
        "name": "Industrial Pipe Bookshelf", "category": "storage", "price": 130.0, "currency": "USD",
        "styles": ["industrial", "loft", "vintage"],
        "colors": ["black/rustic brown", "white/natural"],
        "dimensions": {"w": 100, "d": 30, "h": 175, "unit": "cm"},
        "rating": 4.3, "reviews": 1100,
        "image_url": "https://s.alicdn.com/@sc04/kf/Hindustrial_bookshelf.jpg_300x300.jpg",
        "buy_url": "https://www.alibaba.com/product-detail/Industrial-Bookshelf_62534567897.html",
        "description": "5-tier industrial bookshelf with metal pipe frame and wood shelves.",
    },
    {
        "item_id": "ali-upholstered-bed", "source": "alibaba",
        "name": "Upholstered Platform Bed", "category": "bed", "price": 380.0, "currency": "USD",
        "styles": ["modern", "luxury", "contemporary"],
        "colors": ["light grey", "dark grey", "beige", "navy"],
        "dimensions": {"w": 193, "d": 203, "h": 120, "unit": "cm"},
        "rating": 4.5, "reviews": 678,
        "image_url": "https://s.alicdn.com/@sc04/kf/Hupholstered_platform_bed.jpg_300x300.jpg",
        "buy_url": "https://www.alibaba.com/product-detail/Upholstered-Platform-Bed_62534567898.html",
        "description": "King-size upholstered platform bed with tufted headboard.",
    },
    {
        "item_id": "ali-pendant-light", "source": "alibaba",
        "name": "Nordic Pendant Light", "category": "lighting", "price": 45.0, "currency": "USD",
        "styles": ["nordic", "minimalist", "modern"],
        "colors": ["black", "white", "gold", "copper"],
        "dimensions": {"w": 30, "d": 30, "h": 120, "unit": "cm"},
        "rating": 4.4, "reviews": 2300,
        "image_url": "https://s.alicdn.com/@sc04/kf/Hnordic_pendant_light.jpg_300x300.jpg",
        "buy_url": "https://www.alibaba.com/product-detail/Nordic-Pendant-Light_62534567899.html",
        "description": "Minimalist Nordic pendant light with E27 socket.",
    },
    {
        "item_id": "ali-persian-rug", "source": "alibaba",
        "name": "Persian Style Area Rug", "category": "rug", "price": 89.0, "currency": "USD",
        "styles": ["traditional", "classic", "bohemian"],
        "colors": ["red/gold", "blue/ivory", "green/beige"],
        "dimensions": {"w": 160, "d": 230, "h": 0, "unit": "cm"},
        "rating": 4.2, "reviews": 1560,
        "image_url": "https://s.alicdn.com/@sc04/kf/Hpersian_area_rug.jpg_300x300.jpg",
        "buy_url": "https://www.alibaba.com/product-detail/Persian-Area-Rug_62534567900.html",
        "description": "Traditional Persian pattern area rug, machine washable.",
    },
    {
        "item_id": "ali-standing-desk", "source": "alibaba",
        "name": "Electric Height-Adjustable Standing Desk", "category": "desk", "price": 265.0, "currency": "USD",
        "styles": ["modern", "office", "ergonomic"],
        "colors": ["white", "black", "bamboo"],
        "dimensions": {"w": 140, "d": 70, "h": 125, "unit": "cm"},
        "rating": 4.6, "reviews": 3400,
        "image_url": "https://s.alicdn.com/@sc04/kf/Hstanding_desk_electric.jpg_300x300.jpg",
        "buy_url": "https://www.alibaba.com/product-detail/Standing-Desk-Electric_62534567901.html",
        "description": "Electric sit-stand desk with memory presets and anti-collision.",
    },
]

REQUIRED_FIELDS = ["item_id", "source", "name", "category", "price", "currency",
                   "styles", "colors", "dimensions", "rating", "reviews",
                   "image_url", "buy_url", "description"]

CSV_FIELDS = ["item_id", "source", "name", "category", "price", "currency",
              "rating", "reviews", "styles", "colors", "description"]


# ---------------------------------------------------------------------------
# Validation
# ---------------------------------------------------------------------------

def validate_alibaba(items: List[dict] = ALIBABA_ITEMS) -> bool:
    """
    Validate every Alibaba catalog entry for:
    - all required fields present and non-empty
    - price > 0
    - rating in [1, 5]
    - dimensions are positive numbers
    - item_id starts with 'ali-'
    - buy_url starts with 'https://www.alibaba.com'
    """
    errors = []
    seen_ids: set = set()

    for item in items:
        iid = item.get("item_id", "<unknown>")

        # Duplicate IDs
        if iid in seen_ids:
            errors.append(f"{iid}: duplicate item_id")
        seen_ids.add(iid)

        # Required fields
        for field in REQUIRED_FIELDS:
            val = item.get(field)
            if val is None or val == "" or val == [] or val == {}:
                errors.append(f"{iid}: missing/empty '{field}'")

        # Price
        price = item.get("price", 0)
        if not isinstance(price, (int, float)) or price <= 0:
            errors.append(f"{iid}: invalid price={price}")

        # Rating
        rating = item.get("rating", 0)
        if not isinstance(rating, (int, float)) or not (1.0 <= rating <= 5.0):
            errors.append(f"{iid}: invalid rating={rating}")

        # Dimensions
        dims = item.get("dimensions", {})
        for dim_key in ("w", "d"):
            v = dims.get(dim_key, 0)
            if not isinstance(v, (int, float)) or v < 0:
                errors.append(f"{iid}: invalid dimension '{dim_key}'={v}")

        # item_id prefix
        if not str(iid).startswith("ali-"):
            errors.append(f"{iid}: item_id must start with 'ali-'")

        # buy_url
        buy_url = item.get("buy_url", "")
        if not buy_url.startswith("https://www.alibaba.com"):
            errors.append(f"{iid}: buy_url must start with 'https://www.alibaba.com'")

    if errors:
        print("[validate_alibaba] FAILED:")
        for e in errors:
            print(f"  ✗ {e}")
        return False

    print(f"[validate_alibaba] OK — {len(items)} items, all checks passed ✓")
    return True


# ---------------------------------------------------------------------------
# Builders
# ---------------------------------------------------------------------------

def build_catalog(output: Path = Path("datasets/furniture.csv")) -> None:
    """Write combined IKEA + Alibaba catalog to CSV."""
    output.parent.mkdir(parents=True, exist_ok=True)
    all_items = IKEA_ITEMS + ALIBABA_ITEMS

    with output.open("w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=CSV_FIELDS)
        writer.writeheader()
        for item in all_items:
            writer.writerow({
                "item_id":     item["item_id"],
                "source":      item["source"],
                "name":        item["name"],
                "category":    item["category"],
                "price":       item["price"],
                "currency":    item["currency"],
                "rating":      item["rating"],
                "reviews":     item["reviews"],
                "styles":      "|".join(item.get("styles", [])),
                "colors":      "|".join(item.get("colors", [])),
                "description": item.get("description", ""),
            })

    print(f"[build_furniture] wrote {len(all_items)} items "
          f"({len(IKEA_ITEMS)} IKEA + {len(ALIBABA_ITEMS)} Alibaba) -> {output}")


def build_json(output: Path = Path("datasets/furniture.json")) -> None:
    """Write full combined catalog to JSON (all fields)."""
    output.parent.mkdir(parents=True, exist_ok=True)
    all_items = IKEA_ITEMS + ALIBABA_ITEMS
    output.write_text(json.dumps(all_items, indent=2, ensure_ascii=False))
    print(f"[build_furniture] wrote full JSON -> {output}")


if __name__ == "__main__":
    ok = validate_alibaba()
    build_catalog()
    build_json()
    if not ok:
        sys.exit(1)
